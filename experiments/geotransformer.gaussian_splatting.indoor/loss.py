...
class Evaluator(nn.Module):
    ...
    def evaluate_registration(self, output_dict, data_dict):
        transform = data_dict['transform']
        transform[:3,:3] = transform[:3,:3] / data_dict['ref_adjust_scale'] * data_dict['src_adjust_scale']
        transform[:3, 3] = transform[:3, 3] / data_dict['ref_adjust_scale']

        est_transform = output_dict['estimated_transform']
        est_transform[:3,:3] = est_transform[:3,:3] / data_dict['ref_adjust_scale'] * data_dict['src_adjust_scale']
        est_transform[:3, 3] = est_transform[:3, 3] / data_dict['ref_adjust_scale'] + data_dict['ref_center'] - torch.matmul(est_transform[:3,:3], data_dict['src_center'])

        src_points = output_dict['src_points']

        rre, rte, rse = isotropic_transform_error(transform, est_transform)
        ...
        return rre, rte, rse, rmse, recall

    def forward(self, output_dict, data_dict):
        rre, rte, rse, rmse, recall = self.evaluate_registration(output_dict, data_dict)
