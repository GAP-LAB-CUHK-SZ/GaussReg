...
class SuperPointTargetGenerator(nn.Module):
    ...
    def forward(self, gt_corr_indices, gt_corr_overlaps):
        ...
        if gt_corr_masks.sum() == 0:
            gt_corr_masks = torch.zeros_like(gt_corr_overlaps).to(torch.bool)
            gt_corr_masks = torch.where(gt_corr_overlaps == gt_corr_overlaps.max(), True, gt_corr_masks)
            # gt_corr_masks[gt_corr_overlaps == gt_corr_overlaps.max()] = True
            # gt_corr_masks = torch.ge(gt_corr_overlaps, -0.05)
