...
def weighted_procrustes(
    ...
    # s_src_points = torch.sum(weights * torch.norm(src_points_centered, p=2, dim=2, keepdim=True), dim=1, keepdim=False) # (B, 1)
    # s_ref_points = torch.sum(weights * torch.norm(ref_points_centered, p=2, dim=2, keepdim=True), dim=1, keepdim=False) # (B, 1)
    # src_points_centered = src_points_centered / (s_src_points[:,:,None] + 1e-8) # (B, N, 3)
    # ref_points_centered = ref_points_centered / (s_ref_points[:,:,None] + 1e-8) # (B, N, 3)
    ...
    # R = R * s_ref_points[:,:,None] / (s_src_points[:,:,None] + 1e-8)
    # t = t * s_ref_points
