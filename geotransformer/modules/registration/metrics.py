...
from geotransformer.modules.ops import apply_transform, pairwise_distance, get_rotation_translation_from_transform_w_scale
...
def relative_translation_error(gt_translations, translations):
    ...
    Returns:
        rre (Tensor): relative rotation errors (*)
    """
    rte = torch.linalg.norm(gt_translations - translations, dim=-1) / torch.linalg.norm(gt_translations, dim=-1)
    return rte

def relative_scale_error(gt_scale, est_scale):
    ...
    """
    return torch.linalg.norm(gt_scale - est_scale, dim=-1) / torch.linalg.norm(gt_scale, dim=-1)
...
def isotropic_transform_error(gt_transforms, transforms, reduction='mean'):
    ...
    gt_rotations, gt_translations, gt_scales = get_rotation_translation_from_transform_w_scale(gt_transforms)
    rotations, translations, scales = get_rotation_translation_from_transform_w_scale(transforms)

    rre = relative_rotation_error(gt_rotations, rotations)  # (*)
    rte = relative_translation_error(gt_translations, translations)  # (*)
    rse = relative_scale_error(gt_scales, scales)  # (*)
...
def isotropic_relative_transform_error(gt_transforms, transforms, reduction='mean'):
    r"""Compute the isotropic Relative Rotation Error and Relative Translation Error.
    ...
    Returns:
        rre (Tensor): relative rotation error.
        rte (Tensor): relative translation error.
        rse (Tensor): relative scale error.
    """
    assert reduction in ['mean', 'sum', 'none']

    gt_rotations, gt_translations = get_rotation_translation_from_transform_w_scale(gt_transforms)
    rotations, translations = get_rotation_translation_from_transform_w_scale(transforms)
